...
Напишите класс Sub таким образом, чтобы для двух целых параметров (типа int) a, b конструкция
`Sub(a, b).value()` возвращала разность a, b. Конструкция `Sub(Sub(a,b),c).value()` должна возвращать разность
a − b − c.
